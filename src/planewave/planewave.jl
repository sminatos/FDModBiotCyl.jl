...
function taper_initial_field!(field,nz,nr,dz,src_iz,wavelength,ntaper)
    ...
    if (src_iz-wl_half_iz-ntaper<=0)
        error("srcdepth is too shallow or src wavelength is too large!")
    elseif (src_iz+wl_half_iz+ntaper>nz)
        error("srcdepth is too deep or src wavelength is too large!")
    end
 ...
 #  src_func_forStress=(src_func[1:end]+[0; src_func[1:end-1]])/2.0
   ...
   for ir=1:nr-noffset
      ...
      for iw=2:if_max
         ...
         A0,B0,C0=SolvePengBC(w,angle_delta,rb,Vp_fluid,Vp_solid,Vs_solid,Rho_fluid,Rho_solid)
...
#Assuming open hole
function Schoenberg_solution_initialBC(nr,nz,dr,dz,ir_wall,Vp,
...
#-------------------------------
function initialize_planewave(nr,nz,dr,dz,ir_wall,Rho,H,G,nt,tvec,dt,src_func,srcdepth,f0,delay)
