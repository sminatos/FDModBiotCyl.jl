...
c   SM: Below is original -> phase is constant, weired       
c      primex(if)=2./znorm(ipts)*cexp(ci*wr(if)*z/(vp(1)*(1.+1./pi/qp(1)*
c     *            alog(wr(if)/twopi/fref)-ci/2./qp(1))))
c   SM: One of problems of above 'primex' is that primex(1) may not correspond to zero frequency (i.e., wr(1)/=0),
c     : although later fft seems to assume so.
c     : Therefore, here I fill 'primex' and 'spect' from if=2      
C   SM: (Important) From Tsang and Rader (1979), Tang and Cheng (text book), and Bouchon and Schmitt (1989) ->
C     : Below my 'primex' is defined as freq-domain representation of 1/R*X(t-R/Vf).
C     : Correct scaling due to definition of FT may be nessesary --> *1/2pi.
C   SM: A0'(A0prime=xkernal) should have been scaled correclty (see xkintgrl_SM02.f).      
...
c SM: Filling "if" from 2, for the same reason as primex. This requires to check the value of wr(1) and those in gfcn
         spect(if+1)=gfcn(ipts,if)*source(if)*tshft
         primex(if+1)=primex(if+1)*source(if)*tshft
