...
C     SM: Playing with this scaling, assuming that direct field (appeared in trae_time_SM01.f) is not claculated
C        from FFT, but 1/R*exp(ikR)
        
   51 do 61 iscale=1,nk
C     original
C     xkernal(iscale)=xkernal(iscale)*scale
C     : SM. interprating textbook from Tang-Cheng
C     : pp32. phid=exp(ikf*R)/R, phid=1/pi integral(K0(fr)*exp(ikz))dk         
C     : So, exp(ikf*R)/R assumes source to be 1/pi*K0, and here we assume K0.
C     : We scale A0' by ...1/pi!         
