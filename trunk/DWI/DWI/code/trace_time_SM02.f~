...
c--SM01: playing with phase and sign of primex and spect
c     : seems like primex has very small amplitude compared to spect.
c     : Is this OK?? 
...
c     SM: this is just 2/z*exp(iwr/Vp) : no scaling like rhof,pi, is this ok??

c---original---: phase is constant , weired       
c      primex(if)=2./znorm(ipts)*cexp(ci*wr(if)*z/(vp(1)*(1.+1./pi/qp(1)*
c     *            alog(wr(if)/twopi/fref)-ci/2./qp(1))))
c--SM: just 2/z*exp(iz/Vp)
c      primex(if)=2./(znorm(ipts)*rnorm)*
c     *     exp(ci*wr(if)*(znorm(ipts)*rnorm)/
c     *      (vp(1)*(1.+1./pi/qp(1)*
c     *            alog(wr(if)/twopi/fref)-ci/2./qp(1))))
c--   SM: further problem is primex(1) may not be zero frequency (i.e., wr(1)/=0), although later in fft it seems assumes so
c       : therefore, here I fill primex and spect from 2      
c      primex(if+1)=2./(znorm(ipts)*rnorm)*
c     *     cexp(ci*wr(if)*(znorm(ipts)*rnorm)/
c     *      (vp(1)))
c      primex(if)=1./2.0*
c     *     cexp(ci*wr(if)*2.0/1.5)
c--   SM: I do not understand why I need factor of 2-> remove. what is the relation between pulse and K0 integral?
c     : Tsan and Rader 1979 show that between pulse and Hankel function...
c     : gfcn was scaled considering this (getf or xkintgrl.f), as 2/pi, but why,,,
C     : Fixed primex as 1/R*exp(ikR), and try to set correct scaling for A0prime (xkintgrl_SM01.f)   
C     : (important) See Tsang and Rader (1979). Below is freq-domain representation of
C     : 1/R*X(t-R/V). Correct scaling due to definition of FFT is nessesary --> 1/2pi.
C     : According to this, scaling of A0' in xkintgrl_SM01.f should be considered.
C     : I used textbook from Tang and Cheng (Textbook)      
      primex(if+1)=1./(2.0*3.1415927)/(znorm(ipts)*rnorm)*
     *     cexp(ci*wr(if)*(znorm(ipts)*rnorm)/
     *      (vp(1)))

      
      write(*,*) if,wr(if),real(primex(if)),imag(primex(if)),
     * real(source(if)),imag(source(if))
c**********************************************
c**************APPLY SOURCE******************

c ----- multiply Green's function by the source spectrum

c---following is an original
c         spect(if)=gfcn(ipts,if)*source(if)*tshft
c         primex(if)=primex(if)*source(if)*tshft
c------
         
c---  SM
C     it seems that even tshft ==1., response in homogenous media has wiered phase rotation.
c         
c         primex(if)=primex(if)*source(if)
c     write(*,*) "if=",if,tshft
C     Therefore, I assume t0==0, and tshift=1.
         if (t0 .ne. 0.) then
            write(*,*) "Error! t0 is not zero! I assume t0 to be zero!"
         endif
c Original: Source function (Ricker) will be d/d^2 in the final direct waves. reflect waves are not consistent to FD...         
c         spect(if)=gfcn(ipts,if)*source(if)
c         primex(if)=primex(if)*source(if)
c A: change sign of reflect, and source function will be same in the final direct waves (thus twice integration)
c     spect(if)=-gfcn(ipts,if)*source(if)
c B: filling from 2, for same reason as primex. This requires to check the value of wr(1) and those in gfcn
         spect(if+1)=gfcn(ipts,if)*source(if)*1.0
         primex(if+1)=primex(if+1)*source(if)*1.0

c      write(*,*) if,wr(if),real(primex(if)),imag(primex(if))         
c---SM END        
...
c--   sm---
c        do i=1,nf
c           write(*,*) i,wr(i),real(source(i)),imag(source(i))
c           write(*,*) i,wr(i),real(primex(i)),imag(primex(i))
c        enddo
c----------        
