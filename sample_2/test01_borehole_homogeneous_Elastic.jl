...
#   Rho2_elastic=2000.0
   ...
   Vp2_elastic=2500.0 #Or, Specify K_elastic
   Vs2_elastic=1000.0
   Rho2_elastic=1000.0

   ir_wall=nr
   for iz=171:172 #Elastic media
   ...
   #==Force Enclosing by solid media (TEST)
   ir_wall=11
   for iz=1:nz #Elastic media
       ...
       Flag_E[iz,ir_wall+1:nr]=ones(nr-ir_wall)
       Rho[iz,ir_wall+1:nr]=ones(nr-ir_wall)*Rho2_elastic #Rho=Rho_elastic
       Rhof[iz,ir_wall+1:nr]=ones(nr-ir_wall)*Rho2_elastic #Rhof=Rho_elastic
       G[iz,ir_wall+1:nr]=ones(nr-ir_wall)*G_elastic #G=Gs (Ou's)
       #--
       Phi[iz,ir_wall+1:nr]=ones(nr-ir_wall)*0.0 #Phi=0
       Km[iz,ir_wall+1:nr]=ones(nr-ir_wall)*K_elastic #Km
       Ks[iz,ir_wall+1:nr]=ones(nr-ir_wall)*K_elastic #Ks=Km (-> M=Inf, Ou's)
       #--
       Kappa0[iz,ir_wall+1:nr]=ones(nr-ir_wall)*0.0 #k0=0 (-> D1=D2=Inf, Ou's)
       #
       Flag_AC[iz,ir_wall+1:nr]=ones(nr-ir_wall)*0.0
   end
   ==#
   #Force elastic (OK)
   Flag_E=ones(nz,nr)
...
#   get_snapshots_t!(snapshots_trr,cnt_snap,pf,nr,nz)
   get_snapshots_t!(snapshots_trr,cnt_snap,trz,nr,nz)
