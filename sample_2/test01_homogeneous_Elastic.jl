...
for iz=251:nz #Elastic media
    ...
    Flag_E[iz,1:nr]=ones(nr)
    Rho[iz,1:nr]=ones(nr)*Rho2_elastic #Rho=Rho_elastic
    Rhof[iz,1:nr]=ones(nr)*Rho2_elastic #Rhof=Rho_elastic
    G[iz,1:nr]=ones(nr)*G_elastic #G=Gs (Ou's)
    #--
    Phi[iz,1:nr]=ones(nr)*0.0 #Phi=0
    Km[iz,1:nr]=ones(nr)*K_elastic #Km
    Ks[iz,1:nr]=ones(nr)*K_elastic #Ks=Km (-> M=Inf, Ou's)
    #--
    Kappa0[iz,1:nr]=ones(nr)*0.0 #k0=0 (-> D1=D2=Inf, Ou's)
    #
    Flag_AC[iz,1:nr]=ones(nr)*0.0
end
...
#Check
rec_vz_Aki=Green_Aki(2500,2000,2500,tvec,src_func,srcgeom,recgeom)
